"""
Motion Library for Imitation Learning / AMP (Adversarial Motion Prior)

This module provides utilities for loading, storing, and querying motion reference data
for humanoid robot imitation learning tasks. It is robot-agnostic and can be used with
any humanoid robot by providing appropriate configuration parameters.

Components:
    - load_imitation_dataset: Load motion capture data from pickle files
    - MotionLib: Motion reference library for sampling and querying motion states
    - compute_residual_observations: Compute residual between current and reference states
"""
...
def load_imitation_dataset(folder, mapping="joint_id.txt", suffix=".npz"):
    """
    Load imitation learning dataset from pickle files.
    
    Args:
        folder: Directory containing motion data files
        mapping: Path to joint ID mapping file
        suffix: File extension filter (default: ".npz")
    
    Returns:
        dataset_list: List of motion data dictionaries
        joint_id_dict: Mapping from joint names to indices
    """
...
class MotionLib:
    """
    Motion Reference Library for Imitation Learning / AMP.
    
    Stores and manages motion capture reference data, providing methods for:
    - Motion sampling and time sampling
    - State interpolation between frames
    - Expert observation generation for AMP discriminator
    
    This class is robot-agnostic - it adapts to different robots through
    the dof_names and keyframe_names parameters.
    """
    
    def __init__(self, datasets, mapping, dof_names, keyframe_names, fps=30, min_dt=0.1, 
                 device="cpu", amp_obs_type='keyframe', frame_skip=1, num_steps=2):
    ...
    def get_expert_obs(self, batch_size):
        """Get expert observations for AMP discriminator."""
...
def compute_residual_observations(motion_dict, base_quat, body_pos, body_quat, body_lin_vel, body_ang_vel):
    """
    Compute residual observations between current state and reference motion.
    
    Args:
        motion_dict: Reference motion state dictionary
        base_quat: Current base quaternion
        body_pos: Current body positions
        body_quat: Current body quaternions
        body_lin_vel: Current body linear velocities
        body_ang_vel: Current body angular velocities
    
    Returns:
        Tuple of residual (position, quaternion, linear velocity, angular velocity)
    """
